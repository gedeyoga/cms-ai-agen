...
const formSchema = toTypedSchema(
    z.object({
        otp: z.array(z.coerce.string()).length(5, {message: 'OTP must contain exactly 5 characters'})
    })
)
...
const startCountdown = (expiredAt: string, onUpdate: Function, onComplete: Function) => {
...
const resendOtp = async () => {
    ...
    if(result.statusCode == 200) {
        ...
        countdownTimer = startCountdown(result.data.userOtp.expiredAt, updateCountdown, handleCountdownComplete)
    }
}
...
onMounted(() => {
    if(dataAuthWhatsapp.value.expiredAt) {
        countdownTimer = startCountdown(dataAuthWhatsapp.value.expiredAt, updateCountdown, handleCountdownComplete)
    }
})
...
<template>
    <div class="pt-10 pb-5">
        <h2 class="dark:text-white text-2xl text-center">Whatsapp Verification</h2>
    </div>
    <form @submit="onSubmit">
        ...
        <FormItem>
            ...
            <FormDescription>
                <span class="mt-5 block text-center">An otp message has been send to your whatsapp message. please input it and proceed</span>
            </FormDescription>
